...
(defn sub-graph-node
  ...
  [{:keys [init-state-function construct-graph next-state]}]
  (fn [state]
    (async/go
      (try
        (let [;; consturuct initial state
              new-conversation-state
              (->
               ((if (coll? init-state-function)
                  (apply-functions init-state-function)
                  init-state-function)
                state)
              ...
              (async/<!
               (stream
                ;; figure out which graph to use (defaults to chat-with-tools
                ((or construct-graph
